...
class MinecraftStopInteraction extends BaseInteraction {
  constructor(boat) {
    const info = {
      name: 'stop',
      enabled: true,
  ...
  async run(interaction: ChatInputCommandInteraction) {
    ...
    let stop = await this.boat.client.minecraftApi.stop().catch((err) => err);

    if (stop !== true) return error(interaction, this.boat, stop)

    interaction.editReply({ content: "The server is stopping!" })
    this.boat.log.warn('minecraft-stop', `${interaction.user.toString()} stopped the server`)
  }
}
...
function error(interaction: ChatInputCommandInteraction, boat: BoatI, err: any) {
  ...
  boat.log.warn(module, `Error occurred during minecraft stop: ${util.formatWithOptions({}, err)}`);
}

export default MinecraftStopInteraction;