...
class MinecraftRestartInteraction extends BaseInteraction {
  constructor(boat) {
    const info = {
      name: 'restart',
      enabled: true,
  ...
  async run(interaction: ChatInputCommandInteraction) {
    ...
    let restart = await this.boat.client.minecraftApi.restart().catch((err) => err);

    if (restart !== true) return error(interaction, this.boat, restart);

    interaction.editReply({ content: "The server is rebooting!" })
    this.boat.log.warn('minecraft-restart', `${interaction.user.toString()} restarted the server`)
  }
}
...
function error(interaction: ChatInputCommandInteraction, boat: BoatI, err: any) {
  ...
  boat.log.warn(module, `Error occurred during minecraft restart: ${util.formatWithOptions({}, err)}`);
}

export default MinecraftRestartInteraction;