import { ButtonInteraction, InteractionCollectorOptions, Message, MessageActionRow, MessageButton, MessageEmbed, MessageAttachment, SnowflakeUtil, CommandInteraction, MessagePayload, MessageEditOptions } from 'discord.js';
...
export const InteractionPaginator = async (interaction: CommandInteraction, data: any, offset = 0, length = 1, callback: ({ data, offset, interaction }: { data: any, offset: number, interaction?: CommandInteraction }) => any, options: InteractionCollectorOptions<ButtonInteraction>, editreply: boolean = false) => {
  let currentIndex = offset;
